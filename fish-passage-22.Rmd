...
sort <- c(
  "discharge-simple", "discharge",
  "baseline", "precip", "snowmelt", "obs_vs_preds",
...
The primary goal of the current analyses is to answer the following questions:
  ...
  > How can we model stream temperature to include spatial correlation through the stream network?
  
### Data Preparation
  
Sub-hourly water temperature data collected in the Nechako Watershed in northern British Columbia between 2019 and 2021 were downloaded as csv files from [here]([https://zenodo.org/records/6426024#.ZEAqr-zMI0Q) [@morris_sub-hourly_2022].
...
- All stream temperature data were included, except those flagged "Fail" (see @gilbert_sub-hourly_2022 for details).
...
Where computationally practical, the sensitivity of the posteriors to the choice of prior distributions was evaluated by doubling the standard deviations of the priors and then using $\hat{R}$ to evaluate whether the samples were drawn from the same posterior distribution [@thorley_fishing_2017].
...
In general, continuous and discrete fixed variables are held constant at their arithmetic mean and first level values, respectively, while random effects are held constant at their typical value [@kery_bayesian_2011, pp. 77-82].
Unless stated otherwise the typical value is the arithmetic mean.
When informative the influence of a particular variables is expressed in terms of the *effect size* (i.e., relative change in the response variable) with the 95% CI [@bradford_using_2005].
...
The data were analysed using a Spatial Stream Network model [@ver_hoef_moving_2010; @peterson_mixedmodel_2010], using code adapted from the [`SSNbayes`](https://github.com/EdgarSantos-Fernandez/SSNbayes) package [@santos-fernandez_bayesian_2022].
The necessary stream network distances were computed using the BC Freshwater Atlas. 
...
\begin{equation}
T_s = \frac{\alpha}{1 + e^{\gamma(\beta - T_a)}}
\end{equation}

where $T_s$ is the stream temperature and $T_a$ is the air temperature (both in ˚C), $\alpha$ is the maximum stream temperature, $\gamma$ is a measure of the steepest slope of the function, and $\beta$ is the air temperature at the inflection point of the curve.

Growing Season Degree Days (GSDD) are the accumulated thermal units (in ˚C) during the growing season based on the mean daily water temperature values, and is a useful predictor of age-0 trout size at the beginning of winter.
The start and end of the growing season were defined based on the definitions of Coleman and Fausch @coleman_cold_2007:
...
- The covariance structure of the residual variation combines the following covariance components:
  ...
  - Exponential tail-down (allows spatial dependence between flow-connected and flow-unconnected locations)
