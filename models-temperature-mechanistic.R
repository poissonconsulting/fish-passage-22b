...
model <- model(
  code = read_file("temperature-mechanistic.stan"),
  ...
  new_expr = "
    for (i in 1:nObs) {
      if (week[i] == 1) {
        ...
        eIceReserve[i] = bInitialIceReserve
      } else {
