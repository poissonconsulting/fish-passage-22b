...
sbf_set_sub("clean", "dewpoint-temp")
sbf_load_datas()
...
closest_site_water_temp <- 
  ps_nearest_feature(water_temp_site, dewpoint_temp_site, dist_col = "dist") %>% 
  as_tibble() %>% 
...
dewpoint_temp %<>%
  ...
  select(
    cell_id, site, dewpoint_temp, date, time
  )

rm(max_dist, closest_site_water_temp, water_temp_site)

sbf_set_sub("tidy", "dewpoint-temp", rm = TRUE)
sbf_save_datas()
