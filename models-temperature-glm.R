...
model <- model(
  code = read_file("temperature-glm.stan"),
  ...
  new_expr = "
    for (i in 1:nObs) {
      eTemp[i] <- bIntercept + bDischarge * discharge[i] + bSolar * net_solar_rad[i] + bElev * elev[i] + bAirTemp * air_temp[i]
      # imputed_temp[i] <- y[i]
  ...
  select_data = list(
    ...
    `net_solar_rad/` = c(1e4, 1e9), # divides by sd
    `elev/` = c(500, 1000),
    air_temp = c(-30, 30),
    H = c(0, 50 * 5000),
    E = c(0, 50 * 500)
