...
transformed parameters {
  ...
  for (i in 1:nsite) {
   phi[i] = inv_logit(bPhiIntercept + bPhiSite[i]); 
  }
...
model {
  // Assume positive temporal correlation
  sPhiSite ~ exponential(1);
  bPhiIntercept ~ normal(2, 2);
  for (i in 1:nsite) {
    bPhiSite[i] ~ normal(0, sPhiSite);
  }
