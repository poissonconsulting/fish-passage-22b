...
model <- model(
  code = read_file("temperature-heat.stan"),
  ...
  new_expr = "
    for (i in 1:nObs) {
      ...
      eLongwave[i] <- (0.97 * 5.670367e-8 * (air_temp[i] + 273.15)^4)
      eHeat[i] <- eShortwave[i] + eLongwave[i]
      eTemp[i] <- eHeat[i] / (discharge[i] * 41.8e3 * 1000)
      # eTemp[i] <- (net_solar_rad[i] * 24 * bScale) / (discharge[i] * 41.8e3 * 1000)
      # imputed_temp[i] <- y[i]
