...
ggplot() + 
  ...
  geom_sf(data = points, shape = 21, aes(fill = type))
...
ggplot() +
  ...
  geom_sf(data = network_intersection_points_all) +
  geom_sf(data = points, shape = 21, aes(fill = type)) + 
  NULL
...
gp <- ggplot() +
  ...
  scale_fill_disc_poisson(order = c(1, 3)) +
  labs(fill = "Site Type") +
...
# Ensure segments sites are on are not downstream
connectivity_ps %<>% 
  left_join(points %>% select(base = site, segment_id), join_by(base)) %>% 
  mutate(downstream = if_else(compare == segment_id, FALSE, downstream))
...
sbf_set_sub("distance", "discharge")
...
## Extract just the prediction points and save them for the temperature model
temp_points <- 
  points %>% 
  as_tibble() %>% 
  filter(type == "pred") %>% 
...
downstream_hydrologic_distance <- 
  downstream_hydrologic_distance[temp_points, temp_points]
total_hydrologic_distance <- 
  total_hydrologic_distance[temp_points, temp_points]
euclidean_distance <-
  euclidean_distance[temp_points, temp_points]
weight_matrix <-
  weight_matrix[temp_points, temp_points]
flow_connected <-
  flow_connected[temp_points, temp_points]
