library(openSTARS)
source("load-rgrass.R")

grass_program_path <- "/Applications/GRASS-7.8.app/Contents/Resources/"

grass_db_path <- file.path(getwd(), "output/objects/ssn")
...
initGRASS <- function (gisBase, home, SG, gisDbase, addon_base, location, 
  ...
  if (SYS == "WinNat") {
    ...
    if (missing(addon_base)) 
      addon_base <- paste(Sys.getenv("APPDATA"), "/GRASS7/addons", 
                          sep = "")
    addon_res <- file.exists(addon_base, paste(addon_base, 
                                               "/bin", sep = ""))
    if (any(addon_res)) 
    ...
    if (nchar(OSGEO4W_ROOT) > 0) {
      ...
      if (length(grep("OSGEO4W.*/APPS/GRASS", unistring)) > 
          0) 
    ...
    if (length(grep(basename(Sys.getenv("GISBASE")), Wpath)) < 
        1) {
      ...
      Sys.setenv(PATH = paste(Sys.getenv("GISBASE"), "\\extrabin;", 
                              Sys.getenv("PATH"), sep = ""))
      if (addon_res[2]) 
        Sys.setenv(PATH = paste(Sys.getenv("GRASS_ADDON_BASE"), 
                                "\\bin;", Sys.getenv("PATH"), sep = ""))
      ePyPATH <- Sys.getenv("PYTHONPATH")
      if ((length(grep(basename(Sys.getenv("GISBASE")), 
                       ePyPATH)) < 1) || nchar(ePyPATH) == 0) {
      ...
      if (nchar(OSGEO4W_ROOT) > 0) {
        Sys.setenv(PYTHONHOME = paste(OSGEO4W_ROOT, "apps/Python37", 
                                      sep = "/"))
      }
      else {
    ...
    gisrc <- ifelse(use_g.dirseps.exe, system(paste("g.dirseps.exe -g", 
                                                    shQuote(Sys.getenv("GISRC"))), intern = TRUE), Sys.getenv("GISRC"))
    ...
    if (missing(addon_base)) 
      addon_base <- paste(Sys.getenv("HOME"), "/Library/GRASS/7.8/Modules", sep = "")
    addon_res <- file.exists(addon_base, paste(addon_base, 
                                               "/bin", sep = ""), paste(addon_base, "/scripts", 
                                                                        sep = ""))
    if (any(addon_res)) 
      Sys.setenv(GRASS_ADDON_BASE = addon_base)
    ePATH <- Sys.getenv("PATH")
      ...
      Sys.setenv(PATH = paste(Sys.getenv("GISBASE"), "/bin:", 
                              Sys.getenv("GISBASE"), "/scripts", ifelse(addon_res[2], 
                                                                        paste(":", Sys.getenv("GRASS_ADDON_BASE"), 
                                                                              "/bin", sep = ""), ""), ifelse(addon_res[3], 
                                                                                                             paste(":", Sys.getenv("GRASS_ADDON_BASE"), 
                                                                                                                   "/scripts", sep = ""), ""), ifelse(nchar(ePATH) == 
                                                                                                                                                        0, "", ":"), ePATH, sep = ""))
    #}
    eLDPATH <- Sys.getenv("LD_LIBRARY_PATH")
    if (length(grep(basename(Sys.getenv("GISBASE")), eLDPATH)) < 
        1) {
    ...
    if (file.exists(Sys.getenv("GISRC")) && !override) 
      stop("A GISRC file already exists; to override, set override=TRUE")
    ePyPATH <- Sys.getenv("PYTHONPATH")
    if (length(grep(basename(Sys.getenv("GISBASE")), ePyPATH)) < 
        1 || nchar(ePyPATH) == 0) {
      GrPyPATH <- paste(Sys.getenv("GISBASE"), "etc", "python", 
                        sep = "/")
      if (nchar(ePyPATH) > 0) {
  ...
  system(paste(paste("g.gisenv", get("addEXE", envir = .GRASS_CACHE), 
                     sep = ""), shQuote(paste("set=GISDBASE=", gisDbase))))
  if (missing(location)) 
  ...
  system(paste(paste("g.gisenv", get("addEXE", envir = .GRASS_CACHE), 
                     sep = ""), shQuote(paste("set=GISDBASE", gisDbase, sep = "="))))
  system(paste(paste("g.gisenv", get("addEXE", envir = .GRASS_CACHE), 
                     sep = ""), shQuote(paste("set=LOCATION_NAME", location, 
                                              sep = "="))))
  system(paste(paste("g.gisenv", get("addEXE", envir = .GRASS_CACHE), 
                     sep = ""), shQuote(paste("set=MAPSET", mapset, sep = "="))))
  system(paste(paste("g.gisenv", get("addEXE", envir = .GRASS_CACHE), 
                     sep = ""), shQuote("set=GRASS_GUI=text")))
  Sys.setenv(GISBASE = gisBase)
  ...
  if (grass_python == "") {
    if (nchar(OSGEO4W_ROOT) > 0) {
      if (file.exists(paste(OSGEO4W_ROOT, "bin/python3.exe", 
                            sep = "/"))) {
        Sys.setenv(GRASS_PYTHON = paste(OSGEO4W_ROOT, 
                                        "bin/python3.exe", sep = "/"))
      }
      else {
        Sys.setenv(GRASS_PYTHON = paste(OSGEO4W_ROOT, 
      ...
      if (strsplit(system(paste("g.version", get("addEXE", 
                                                 envir = .GRASS_CACHE), " -g ", sep = ""), intern = TRUE)[1], 
                   "=")[[1]][2] > "7.6.1") 
        Sys.setenv(GRASS_PYTHON = paste("python3", get("addEXE", 
                                                       envir = .GRASS_CACHE), sep = ""))
      else Sys.setenv(GRASS_PYTHON = paste("python", get("addEXE", 
                                                         envir = .GRASS_CACHE), sep = ""))
    }
  }
  ...
  if (mSG) {
    if (nzchar(wkt_SG)) {
      ...
      execGRASS("g.region", flags = "s", region = paste0("input@", 
                                                         mapset), ignore.stderr = ignore.stderr)
...
initGRASS(gisBase = gisBase, SG = dem_grid, mapset = "PERMANENT", override = TRUE)
execGRASS("g.proj", flags = c("c", "quiet"), parameters = list(georef = dem))
