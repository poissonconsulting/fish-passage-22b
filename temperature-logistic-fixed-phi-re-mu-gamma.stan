...
parameters {
  ...
  // Correlated random effects for mu and gamma
  // first element is mu, second is gamma
  vector<lower=0>[2] sLogisticPars;
  cholesky_factor_corr[2] bLRhoLogisticPars;
  matrix[2, nsite] eZLogisticPars;
...
transformed parameters {
  ...
  matrix[nsite, 2] bLogisticPars;
  matrix[2, 2] bRhoLogisticPars;
  
  vector[nsite * nweek] eMu;
  ...
  for (i in 1:(nweek * nsite)) {
    eMu[i] = bMuIntercept + bLogisticPars[site[i], 1];
    eGamma[i] = bGammaIntercept + bLogisticPars[site[i], 2];
    eTemp[i] = eMu[i] + (bAlpha - eMu[i]) / (1 + exp(eGamma[i] * (bBeta - air_temp[i])));
  }
