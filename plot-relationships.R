...
gp <- ggplot(data %>% filter(!is.na(water_temp))) +
  geom_point(aes(x = net_solar_rad, y = air_temp, colour = as.numeric(week))) +
...
gp <- ggplot(data %>% filter(!is.na(water_temp))) +
  geom_point(aes(x = net_solar_rad, y = discharge, colour = water_temp)) +
  xlab("Net Solar Radiation") +
  ylab("Discharge") +
  NULL
...
gp <- ggplot(data %>% filter(!is.na(water_temp))) +
  geom_point(aes(x = dayte, y = net_solar_rad, colour = discharge)) +
  facet_wrap(~site) +
  NULL
...
gp <- ggplot(data %>% filter(!is.na(water_temp))) +
  geom_point(aes(x = net_solar_rad, y = air_temp, colour = water_temp)) +
  facet_wrap(~annual) +
  NULL
...
data %>% 
  ...
  summarize(
    ...
    net_solar_rad = mean(net_solar_rad, na.rm = TRUE)
  ) %>% 
  mutate(
    across(
      c(elev, discharge, water_temp, air_temp, net_solar_rad),
      \(x) scale(x)       
    )
  ) %>% 
  print(n = nrow(.))
...
data %<>% 
  ...
  select(site, week, annual, variable, value, water_temp, dayte) %>% 
  mutate(
